...
const API_BASE = process.env.NEXT_PUBLIC_API_URL || "https://pvms.club/api"/* "http://localhost:8080" */;
    ...
    let response = new ImageResponse(
        ...
        >
            <h1 tw="flex font-semibold text-[#FAF8F6] text-6xl block whitespace-pre my-0">
                {<FaMedal color={accentColor} size={45} />} PvM <span tw={"text-[" + accentColor + "]"}>Leaderboard </span>
            </h1>
            {
                        ...
                        </span>
                        {
                            players.map((player, index) => {
                                ...
                                return (<span tw="px-2 items-end">
                                    ...
                                    <span tw={"pl-1 align-baseline text-3xl font-bold text-[" + accentColor + "]"}>
                                        {player.player.name}
                                    </span>
                                    <span tw="text-transparent">d</span>{/* Intended way to add a margin to spans :thumbsup: */}
